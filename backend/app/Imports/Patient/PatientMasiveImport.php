...
class PatientMasiveImport implements ShouldQueue, SkipsOnFailure, ToModel, WithChunkReading, WithEvents, WithHeadingRow
{
    ...
    public function registerEvents(): array
    {
        return [
            ...
            AfterImport::class => function (AfterImport $event) {
                ...
                $totalRows = (int) Redis::get("import:{$this->batchId}:processed");
                ...
                ErrorCollector::saveErrorsToDatabase($this->batchId, $errors > 0 ? 'failed' : 'success');
                ...
                event(new ImportProgressEvent(
                    ...
                    $errors > 0 ? 'Importación completada con novedades' : 'Importación completada'
                ));
            },
        ];
    }
    ...
    {
        // Con WithHeadingRow, $row trae claves "slug" de la cabecera
        return DB::transaction(function () use ($row) {
            ...
            // Validaciones de fila (si retorna true, hay error y se omite)
    ...
    /** Unifica la creación de errores al contrato de ErrorCollector */
    protected function pushError(int $processedRowIndex, string $columnKey, $value, array $data, string $message, string $code): void
    {
        ...
        ErrorCollector::addError(
            ...
            columnName: $columnKey, // ej. 'documento', 'tipo_de_usuario'
            errorMessage: $message,
            errorType: $code, // ej. 'PATIENT_ROW_002'
    ...
    public function validations(array $row, int $processed, array $data): bool
    {
        ...
        if (!$this->tipoIdPisis($row['tipo_de_documento'] ?? null, 'id')) {
            $this->pushError($processed, 'tipo_de_documento', $row['tipo_de_documento'] ?? null, $data, 'El ID de Tipo de Documento no existe.', 'PATIENT_ROW_001');
            $error = true;
        }
        ...
        $doc = ($row['documento'] ?? '');
        if (trim($doc) === '') {
            $this->pushError($processed, 'documento', $doc, $data, 'El documento es obligatorio.', 'PATIENT_ROW_002');
            $error = true;
        } elseif (strlen($doc) < 4 || strlen($doc) > 20) {
            $this->pushError($processed, 'documento', $doc, $data, 'El documento debe tener entre 4 y 20 caracteres.', 'PATIENT_ROW_003');
            $error = true;
        }

        // tipo de usuario
        if (!$this->ripsTipoUsuario($row['tipo_de_usuario'] ?? null, 'id')) {
            $this->pushError($processed, 'tipo_de_usuario', $row['tipo_de_usuario'] ?? null, $data, 'Tipo de usuario no válido.', 'PATIENT_ROW_004');
            $error = true;
        }
        ...
        if ($birthDate) {
            ...
            if ($age >= 18 && $originCode === 'CO' && $tipoIdCode !== 'CC') {
                $this->pushError($processed, 'tipo_de_documento', $tipoId, $data, 'Mayores de edad colombianos deben usar CC.', 'PATIENT_ROW_005');
                $error = true;
            }
            if ($tipoIdCode === 'TI' && ($age < 7 || $age > 17)) {
                $this->pushError($processed, 'tipo_de_documento', $tipoId, $data, 'TI válida entre 7 y 17 años.', 'PATIENT_ROW_006');
                $error = true;
            }
            if (in_array($tipoIdCode, ['RC', 'CN'], true) && $age >= 7) {
                $this->pushError($processed, 'tipo_de_documento', $tipoId, $data, 'RC o CN solo para menores de 7 años.', 'PATIENT_ROW_007');
                $error = true;
            }
            if ($tipoIdCode === 'CN' && $age > 3) {
                $this->pushError($processed, 'tipo_de_documento', $tipoId, $data, 'CN recomendado solo hasta 3 años.', 'PATIENT_ROW_008');
                $error = true;
            }
            if ($originCode === 'VE' && $tipoIdCode !== 'PE') {
                $this->pushError($processed, 'tipo_de_documento', $tipoId, $data, 'Venezolanos deben usar PE.', 'PATIENT_ROW_009');
                $error = true;
            }
            ...
            if ($isForeigner && $isTransient && !in_array($tipoIdCode, $allowedForeignTransientDocs, true)) {
                $this->pushError($processed, 'tipo_de_documento', $tipoId, $data, 'Extranjeros de paso: CE, CD, PA o SC.', 'PATIENT_ROW_010');
                $error = true;
            }
        } else {
            $this->pushError($processed, 'fecha_de_nacimiento', $row['fecha_de_nacimiento'] ?? null, $data, 'La fecha no es válida.', 'PATIENT_ROW_011');
            $error = true;
        }

        // sexo
        if (!$this->sexos($row['sexo'] ?? null, 'id')) {
            $this->pushError($processed, 'sexo', $row['sexo'] ?? null, $data, 'Sexo no válido.', 'PATIENT_ROW_012');
            $error = true;
        }

        // país residencia
        if (!$this->paises($row['pais_de_residencia'] ?? null, 'id')) {
            $this->pushError($processed, 'pais_de_residencia', $row['pais_de_residencia'] ?? null, $data, 'País de residencia no válido.', 'PATIENT_ROW_013');
            $error = true;
        }

        // municipio (opcional)
        if (!empty($row['municipio_de_residencia']) && !$this->municipios($row['municipio_de_residencia'], 'id')) {
            $this->pushError($processed, 'municipio_de_residencia', $row['municipio_de_residencia'], $data, 'Municipio de residencia no válido.', 'PATIENT_ROW_014');
            $error = true;
        }

        // zona (opcional)
        if (!empty($row['zona_territorial_de_residencia']) && !$this->zonas($row['zona_territorial_de_residencia'], 'id')) {
            $this->pushError($processed, 'zona_territorial_de_residencia', $row['zona_territorial_de_residencia'], $data, 'Zona no válida.', 'PATIENT_ROW_015');
            $error = true;
        }
        ...
        if (!empty($row['incapacidad'])) {
            ...
            if (!in_array($val, ['SI', 'SÍ', 'NO'], true)) {
                $this->pushError($processed, 'incapacidad', $row['incapacidad'], $data, 'Incapacidad solo acepta SI/SÍ o NO.', 'PATIENT_ROW_016');
                $error = true;
            }
        }
        ...
        if (!$this->paises($row['pais_de_origen'] ?? null, 'id')) {
            $this->pushError($processed, 'pais_de_origen', $row['pais_de_origen'] ?? null, $data, 'País de origen no válido.', 'PATIENT_ROW_017');
            $error = true;
        }
        ...
        if (trim($firstName) === '') {
            $this->pushError($processed, 'primer_nombre', $firstName, $data, 'El primer nombre es obligatorio.', 'PATIENT_ROW_018');
            $error = true;
        } elseif (mb_strlen($firstName) > 255) {
            $this->pushError($processed, 'primer_nombre', $firstName, $data, 'El primer nombre excede 255 caracteres.', 'PATIENT_ROW_019');
            $error = true;
        }

        if (!empty($row['segundo_nombre']) && mb_strlen((string)$row['segundo_nombre']) > 255) {
            $this->pushError($processed, 'segundo_nombre', $row['segundo_nombre'], $data, 'El segundo nombre excede 255 caracteres.', 'PATIENT_ROW_020');
            $error = true;
        }

        $firstSurname = ($row['primer_apellido'] ?? '');
        if (trim($firstSurname) === '') {
            $this->pushError($processed, 'primer_apellido', $firstSurname, $data, 'El primer apellido es obligatorio.', 'PATIENT_ROW_021');
            $error = true;
        } elseif (mb_strlen($firstSurname) > 255) {
            $this->pushError($processed, 'primer_apellido', $firstSurname, $data, 'El primer apellido excede 255 caracteres.', 'PATIENT_ROW_022');
            $error = true;
        }

        if (!empty($row['segundo_apellido']) && mb_strlen((string)$row['segundo_apellido']) > 255) {
            $this->pushError($processed, 'segundo_apellido', $row['segundo_apellido'], $data, 'El segundo apellido excede 255 caracteres.', 'PATIENT_ROW_023');
            $error = true;
        }
