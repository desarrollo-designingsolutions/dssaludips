    ...
    private static function mapRecordToApi(string $serviceTypeKey, array $rec): array
    {
        ...
        if ($k === 'consultas') {
            ...
            return [
                ...
                'codConsulta' => !empty($rec['codConsulta']) ? new CupsRipsSelectInfiniteResource($codConsulta) : null,
                'modalidadGrupoServicioTecSal' => !empty($rec['modalidadGrupoServicioTecSal']) ? new ModalidadAtencionSelectInfiniteResource($modalidad) : null,
                'grupoServicios' => !empty($rec['grupoServicios']) ? new GrupoServicioSelectInfiniteResource($grupo) : null,
                'codServicio' => !empty($rec['codServicio']) ? new ServicioSelectInfiniteResource($servicio) : null,
                'finalidadTecnologiaSalud' => !empty($rec['finalidadTecnologiaSalud']) ? new RipsFinalidadConsultaVersion2SelectInfiniteResource($finalidad) : null,
                'causaMotivoAtencion' => !empty($rec['causaMotivoAtencion']) ? new RipsCausaExternaVersion2SelectInfiniteResource($causa) : null,
                'codDiagnosticoPrincipal' => !empty($rec['codDiagnosticoPrincipal']) ? new Cie10SelectInfiniteResource($diagP) : null,
                'codDiagnosticoRelacionado1' => !empty($rec['codDiagnosticoRelacionado1']) ? new Cie10SelectInfiniteResource($diagR1) : null,
                'codDiagnosticoRelacionado2' => !empty($rec['codDiagnosticoRelacionado2']) ? new Cie10SelectInfiniteResource($diagR2) : null,
                'codDiagnosticoRelacionado3' => !empty($rec['codDiagnosticoRelacionado3']) ? new Cie10SelectInfiniteResource($diagR3) : null,
                'tipoDiagnosticoPrincipal' => !empty($rec['tipoDiagnosticoPrincipal']) ? new RipsTipoDiagnosticoPrincipalVersion2SelectInfiniteResource($tipoDiag) : null,
        ...
        if ($k === 'procedimientos') {
            ...
            return [
                ...
                'numAutorizacion' => $rec['numAutorizacion'] ?? null,
                'codProcedimiento' => !empty($rec['codProcedimiento']) ? new CupsRipsSelectInfiniteResource($codProcedimiento) : null,
                'viaIngresoServicioSalud' => !empty($rec['viaIngresoServicioSalud']) ? new ViaIngresoUsuarioSelectInfiniteResource($viaIngresoServicioSalud) : null,
                'modalidadGrupoServicioTecSal' => !empty($rec['modalidadGrupoServicioTecSal']) ? new ModalidadAtencionSelectInfiniteResource($modalidad) : null,
                'grupoServicios' => !empty($rec['grupoServicios']) ? new GrupoServicioSelectInfiniteResource($grupo) : null,
                'codServicio' => !empty($rec['codServicio']) ? new ServicioSelectInfiniteResource($servicio) : null,
                'finalidadTecnologiaSalud' => !empty($rec['finalidadTecnologiaSalud']) ? new RipsFinalidadConsultaVersion2SelectInfiniteResource($finalidad) : null,
                ...
                'codComplicacion' => !empty($rec['codComplicacion']) ? new Cie10SelectInfiniteResource($complicacion) : null,
                'vrServicio' => $rec['vrServicio'] ?? null,
                'conceptoRecaudo' => !empty($rec['conceptoRecaudo']) ? new ConceptoRecaudoSelectResource($concepto) : null,
                'valorPagoModerador' => $rec['valorPagoModerador'] ?? null,
                'numFEVPagoModerador' => $rec['numFEVPagoModerador'] ?? null,
        ...
        if ($k === 'urgencias') {
            ...
            return [
                ...
                'causaMotivoAtencion' => !empty($rec['causaMotivoAtencion']) ? new RipsCausaExternaVersion2SelectInfiniteResource($causa) : null,
                'codDiagnosticoPrincipal' => !empty($rec['codDiagnosticoPrincipal']) ? new Cie10SelectInfiniteResource($diagP) : null,
                'codDiagnosticoPrincipalE' => !empty($rec['codDiagnosticoPrincipalE']) ? new Cie10SelectInfiniteResource($diagPE) : null,
                'codDiagnosticoRelacionadoE1' => !empty($rec['codDiagnosticoRelacionadoE1']) ? new Cie10SelectInfiniteResource($diagRE1) : null,
                'codDiagnosticoRelacionadoE2' => !empty($rec['codDiagnosticoRelacionadoE2']) ? new Cie10SelectInfiniteResource($diagRE2) : null,
                'codDiagnosticoRelacionadoE3' => !empty($rec['codDiagnosticoRelacionadoE3']) ? new Cie10SelectInfiniteResource($diagRE3) : null,
                'condicionDestinoUsuarioEgreso' => $rec['condicionDestinoUsuarioEgreso'] ?? null,
                'codDiagnosticoCausaMuerte' => !empty($rec['codDiagnosticoCausaMuerte']) ? new Cie10SelectInfiniteResource($diagCausaMuerte) : null,
        ...
        if ($k === 'hospitalizacion') {
            ...
            return [
                'id' => $rec['id'] ?? null,
                'codPrestador' => $rec['codPrestador'] ?? null,
                'viaIngresoServicioSalud' => !empty($rec['viaIngresoServicioSalud']) ? new ViaIngresoUsuarioSelectInfiniteResource($viaIngreso) : null,
                'fechaInicioAtencion' => $rec['fechaInicioAtencion'] ?? null,
                'numAutorizacion' => $rec['numAutorizacion'] ?? null,
                'causaMotivoAtencion' => !empty($rec['causaMotivoAtencion']) ? new RipsCausaExternaVersion2SelectInfiniteResource($causa) : null,
                'codDiagnosticoPrincipal' => !empty($rec['codDiagnosticoPrincipal']) ? new Cie10SelectInfiniteResource($diagP) : null,
                'codDiagnosticoPrincipalE' => !empty($rec['codDiagnosticoPrincipalE']) ? new Cie10SelectInfiniteResource($diagPE) : null,
                'codDiagnosticoRelacionadoE1' => !empty($rec['codDiagnosticoRelacionadoE1']) ? new Cie10SelectInfiniteResource($diagR1) : null,
                'codDiagnosticoRelacionadoE2' => !empty($rec['codDiagnosticoRelacionadoE2']) ? new Cie10SelectInfiniteResource($diagR2) : null,
                'codDiagnosticoRelacionadoE3' => !empty($rec['codDiagnosticoRelacionadoE3']) ? new Cie10SelectInfiniteResource($diagR3) : null,
                'codComplicacion' => !empty($rec['codComplicacion']) ? new Cie10SelectInfiniteResource($complicacion) : null,
                'condicionDestinoUsuarioEgreso' => !empty($rec['condicionDestinoUsuarioEgreso']) ? new CondicionyDestinoUsuarioEgresoSelectInfiniteResource($condicion) : null,
                'codDiagnosticoCausaMuerte' => !empty($rec['codDiagnosticoCausaMuerte']) ? new Cie10SelectInfiniteResource($diagMuerte) : null,
                'fechaEgreso' => $rec['fechaEgreso'] ?? null,
                'consecutivo' => $rec['consecutivo'] ?? null,
            ];
        }
        ...
        if ($k === 'reciennacidos') {
            ...
            return [
                ...
                'codSexoBiologico' => !empty($rec['codSexoBiologico']) ? new SexoSelectResource($sexo) : null,
                'peso' => $rec['peso'] ?? null,
                'codDiagnosticoPrincipal' => !empty($rec['codDiagnosticoPrincipal']) ? new Cie10SelectInfiniteResource($diagP) : null,
                'condicionDestinoUsuarioEgreso' => !empty($rec['condicionDestinoUsuarioEgreso']) ? new CondicionyDestinoUsuarioEgresoSelectInfiniteResource($condicion) : null,
                'codDiagnosticoCausaMuerte' => !empty($rec['codDiagnosticoCausaMuerte']) ? new Cie10SelectInfiniteResource($diagM) : null,
                'fechaEgreso' => $rec['fechaEgreso'] ?? null,
                'consecutivo' => $rec['consecutivo'] ?? null,
            ];
        }
        ...
        if ($k === 'medicamentos') {
            ...
            $nomTec = null;
            if (!empty($rec['nomTecnologiaSalud'])) {
                $nomTec = Dci::where('codigo', $rec['nomTecnologiaSalud'])->first();
            }
            $umm = null;
            ...
            return [
                ...
                'formaFarmaceutica' => !empty($rec['formaFarmaceutica']) ? new FfmSelectResource($ffm) : null,
                'unidadMinDispensa' => !empty($rec['unidadMinDispensa']) ? new UprSelectResource($upr) : null,
