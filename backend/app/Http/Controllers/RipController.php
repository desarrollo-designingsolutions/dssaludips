...
class RipController extends Controller
{
        ...
        return $this->execute(function () use ($id) {
            ...
            if (isset($rip->ripInvoices) && count($rip->ripInvoices) > 0) {
                foreach ($rip->ripInvoices as $key => $value) {
                    ...
                    if ($value['note_type']) {
                        $tipoNota = TipoNota::where('codigo', $value['note_type'])->first();
                    }
                    ...
                    $arrayData[] = [
                        ...
                        'TipoNota' => $value['note_type'] ? new TipoNotaSelectResource($tipoNota) : null,
                        'numNota' => $value['note_number'],
                        'sumVr' => $value['sumVr'],
    ...
    public function getManualInfoUsers($ripInvoice_id)
    {
        return $this->execute(function () use ($ripInvoice_id) {
            ...
            $tipoNota = null;
            if ($ripInvoice->note_type) {
                $tipoNota = TipoNota::where('codigo', $ripInvoice->note_type)->select('codigo', 'nombre')->first();
            ...
            $ripInvoiceInfo = [
                ...
                "tipoNota" => $tipoNota,
                "numNota" => $ripInvoice->note_number,
