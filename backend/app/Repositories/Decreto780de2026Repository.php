...
class Decreto780de2026Repository extends BaseRepository
{
    public function __construct(Decreto780de2026 $modelo)
    {
            ...
            $data = $this->model->with($with)->where(function ($query) {})
                ->where(function ($query) use ($request) {
                    if (isset($request['searchQueryInfinite']) && ! empty($request['searchQueryInfinite'])) {
                        ...
                        $query->orWhere('descripcion', 'like', '%'.$request['searchQueryInfinite'].'%');
                    }
                });
